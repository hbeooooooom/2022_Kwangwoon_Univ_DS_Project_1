...
void Manager::Run(const char* filepath)
{
    ...
    while (!fin.eof())
    {
        ...
        else if (strcmp(command, "ADD") == 0)
        {
            ...
            string num = command;
            int int_num = atoi(num.c_str());
            result = SELECT(BST_list, int_num);
            if (result == Success) {
                cout << "===========SELECT============" << endl;
                cout << "SUCCESS" << endl;
            }
            ...
            if(a=="-l"){
                command = strtok(NULL,"\n");
                string b = command;
                int int_num = atoi(b.c_str());
                result = EDIT(BST_list,a,int_num);
                if(result == Success){
                    cout<<"============EDIT============="<<endl;
                    cout<<"SUCCESS"<<endl;
                }
...
Result Manager::EDIT(Database_BST* BST_list,string cmd, int num){
    Result result;
    result = BST_list->get_picture(cmd,num);
    return result;
}
Result Manager::SELECT(Database_BST* BST_list, int num) {
    Result result;
   result = BST_list->select_bst(BST_list, num);
    if (result!=0){
        return SelectError ;
    }
    return Success;
